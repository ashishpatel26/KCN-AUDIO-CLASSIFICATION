...
model.add(Conv1D(32,17,input_shape=(sampleSize,1), strides=2,padding="same"))
model.add(Activation("relu"))
model.add(Conv1D(32,17,padding="same",strides=2))
model.add(Activation("relu"))
model.add(MaxPool1D(4))
model.add(Dropout(.2))
model.add(Conv1D(64,11,padding="same",strides=2))
model.add(Activation("relu"))
model.add(Conv1D(128,11,padding="same",strides=2))
model.add(Activation("relu"))
model.add(MaxPool1D(4))
...
history= model.fit(allData,labels,epochs=20,batch_size=16,validation_data=(allDataT,labelsT),shuffle=True)
score =  model.evaluate(allData,labels,batch_size=16)
print(model.summary())
print(score)
print(history.history.keys())
model.save("audiolass.h5")
fil = open("rnn-acc.pkl", "wb")
diction = {"model":history.history['accuracy'],"val":history.history['val_accuracy']}
pickle.dump(diction,fil)
fil.close()

fil = open("rnn-loss.pkl", "wb")
